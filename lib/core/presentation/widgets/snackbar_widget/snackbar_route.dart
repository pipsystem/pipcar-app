...
class TopSnackBarRoute<T> extends OverlayRoute<T> {
  ...
  final List<DismissDirection> dismissDirections = const [DismissDirection.horizontal, DismissDirection.up];
  ...
  TopSnackBarRoute({required this.theme, required this.topSnackBar, RouteSettings? settings})
  ...
  @override
  bool get finishedWhenPopped => _animationController?.status == AnimationStatus.dismissed;

  AnimationController createAnimationController() {
    assert(animationDuration >= Duration.zero, 'Can not reuse a $runtimeType if duration = 0');
  ...
  Animation<Alignment> createAnimation() {
    ...
    return AlignmentTween(begin: _initialAlignment, end: _endAlignment).animate(
      CurvedAnimation(
        parent: _animationController ?? AnimationController(vsync: NavigatorState()),
  ...
  void install() {
    ...
    assert(_animationController != null, '$runtimeType.createAnimationController() returned null.');
  ...
  Widget _getDismissibleWidget(
    ...
    for (final direction in dismissDirections) {
      childWidget = Dismissible(
        ...
        direction: direction,
        confirmDismiss: (_) {
          if (currentStatus == TopSnackBarStatus.isAppearing || currentStatus == TopSnackBarStatus.isHiding) {
...
TopSnackBarRoute showSnackBar<T>({required BuildContext context, required TopSnackBar topSnackBar}) {
  return TopSnackBarRoute<T>(
      topSnackBar: topSnackBar, theme: Theme.of(context), settings: const RouteSettings(name: topSnackBarRouteName));
}
